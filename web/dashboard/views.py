        ...
        if mode == 'delete':
            try:
                ...
                messages.add_message(
                    request,
                    messages.INFO,
                    f'Usuaria {user.username} eliminado con éxito.'
                )
                messageData = {'estado': True}
            except Exception as e:
                logger.error(e)
                messageData = {'estado': False}
        elif mode == 'update':
            try:
                ...
                logger.error(e)
                messageData = {'estado': False, 'error': str(e)}
        elif mode == 'create':
            try:
                response = json.loads(request.body)
                if not response.get('password'):
                    messageData = {'estado': False, 'error': 'No se permiten contraseñas vacías'}
...
def on_user_logged_out(sender, request, **kwargs):
    messages.add_message(
        request,
        messages.INFO,
        'Te han registrado con éxito.Gracias ' +
        'por usar reNgine.')
...
def delete_project(request, id):
    ...
    if request.method == "POST":
        ...
        messages.add_message(
            request,
            messages.ERROR,
            '¡Ups!¡El proyecto no se pudo eliminar!')
...
def onboarding(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            error = ' No pudo crear proyecto, Error: ' + str(e)
        ...
        try:
            ...
            error = ' No pudo crear usuario, Error: ' + str(e)
