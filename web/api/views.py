...
class HackerOneProgramViewSet(viewsets.ViewSet):
	...
	def get_api_credentials(self):
		try:
			...
			if not api_key:
				raise ObjectDoesNotExist("Credenciales de la API de Hackerone no encontradas")
			return api_key.username, api_key.key
		except ObjectDoesNotExist:
			raise Exception("Credenciales de la API de Hackerone no configuradas")
	...
	def fetch_programs_from_hackerone(self):
		...
		try:
			...
			raise Exception("Error de credenciales de API: " + str(e))
		...
		while url:
			...
			if response.status_code == 401:
				raise Exception("Credenciales de API inválidas")
			elif response.status_code != 200:
				raise Exception(f"La solicitud de API de Hackerone falló con el código de estado {response.status_code}")
	...
	def refresh_cache(self, request):
		try:
			programs = self.fetch_programs_from_hackerone()
			cache.set(self.CACHE_KEY, programs, self.CACHE_TIMEOUT)
			return Response({"status": "Caché actualizada con éxito"})
		except Exception as e:
			return self.handle_exception(e)
	...
	def import_programs(self, request):
		try:
			...
			if not project_slug:
				return Response({"error": "Se requiere una babosa del proyecto"}, status=status.HTTP_400_BAD_REQUEST)
			handles = request.data.get('handles', [])

			if not handles:
				return Response({"error": "No se proporcionan manijas de programa"}, status=status.HTTP_400_BAD_REQUEST)

			import_hackerone_programs_task.delay(handles, project_slug)

			create_inappnotification(
				title="La importación del programa Hackerone comenzó",
				description=f"Proceso de importación para {len(handles)} El programa (s) ha comenzado.",
			...
			return Response({"message": f"Proceso de importación para {len(handles)} El programa (s) ha comenzado."}, status=status.HTTP_202_ACCEPTED)
		except Exception as e:
			return self.handle_exception(e)
	...
	def sync_bookmarked(self, request):
		try:
			...
			create_inappnotification(
				title="Los programas marcados de Hackerone Sync comenzó",
				description="El proceso de sincronización para programas marcados ha comenzado.",
				notification_type=PROJECT_LEVEL_NOTIFICATION,
				project_slug=project_slug,
				icon="mdi-sync",
				status='info'
			...
			return Response({"message": "El proceso de sincronización para programas marcados ha comenzado."}, status=status.HTTP_202_ACCEPTED)
		except Exception as e:
			return self.handle_exception(e)

	def handle_exception(self, exc):
		if isinstance(exc, ObjectDoesNotExist):
			return Response({"error": "Credenciales de la API de Hackerone no configuradas"}, status=status.HTTP_503_SERVICE_UNAVAILABLE)
		elif str(exc) == "Credenciales de API inválidas":
			return Response({"error": "Credenciales de API de hackerone inválidas"}, status=status.HTTP_401_UNAUTHORIZED)
		else:
...
class GPTAttackSuggestion(APIView):
	def get(self, request):
		...
		if not subdomain_id:
			return Response({
				'status': False,
				'error': 'Faltando el subdominio Get Param `subdomain_id`'
			})
		try:
			...
			return Response({
				'status': False,
				'error': 'Subdominio no encontrado con ID ' + subdomain_id
			})
...
class LLMVulnerabilityReportGenerator(APIView):
	def get(self, request):
		...
		if not vulnerability_id:
			return Response({
				'status': False,
				'error': 'Faltando la vulnerabilidad de Get Param `id`'
			})
...
class WafDetector(APIView):
	def get(self, request):
		...
		if not (validators.url(url) or validators.domain(url)):
			response['message'] = '¡Dominio/URL inválido proporcionado!'
			return Response(response)
		...
		if group:
			...
			response['message'] = '¡No pudo detectar ningún WAF!'
...
class UniversalSearch(APIView):
	def get(self, request):
		...
		if not query:
			response['message'] = '¡No se proporciona un parámetro de consulta!'
			return Response(response)
...
class CVEDetails(APIView):
	def get(self, request):
		...
		if not cve_id:
			return Response({'status': False, 'message': 'CVE ID no proporcionado'})

		response = requests.get('https://cve.circl.lu/api/cve/' + cve_id)

		if response.status_code != 200:
			return  Response({'status': False, 'message': '¡Ocurrió un error desconocido!'})

		if not response.json():
			return  Response({'status': False, 'message': 'CVE ID no existe.'})
...
class ToggleSubdomainImportantStatus(APIView):
	def post(self, request):
		...
		response = {'status': False, 'message': 'No se proporciona subdominio_id'}
...
class AddTarget(APIView):
	...
	def post(self, request):
		...
		if not validators.domain(domain_name):
			return Response({'status': False, 'message': 'Dominio no válido o IP'})
		...
		if status:
			return Response({
				'status': True,
				'message': 'Dominio agregado con éxito como objetivo !',
				'domain_name': domain_name,
				# 'domain_id': domain.id
			})
		return Response({
			'status': False,
			'message': 'No se pudo agregar como objetivo !'
		})
...
class FetchSubscanResults(APIView):
	def get(self, request):
		...
		if not subscan.exists():
			return Response({
				'status': False,
				'error': f'Subescantar {subscan_id} no existe'
			})
...
class StopScan(APIView):
	...
	def post(self, request):
		...
		def abort_scan(scan):
			...
			try:
				logger.info(f"Escaneo {scan} Estado a Aborted_Task")
		...
		def abort_subscan(subscan):
			...
			try:
				logger.info(f"Escaneo {subscan} Estado a Aborted_Task")
...
class InitiateSubTask(APIView):
	...
	def post(self, request):
		...
		for subdomain_id in data['subdomain_ids']:
			logger.info(f'runningSubscans {scan_types} on ssubdominio"{subdomain_id}" ...')
...
class RengineUpdateCheck(APIView):
	def get(self, request):
		...
		if 'message' in response:
			return Response({'status': False, 'message': 'Ratelimitado'})
		...
		create_inappnotification(
			...
			description=f'Actualización a la versión {latest_version} está disponible',
...
class UninstallTool(APIView):
	...
	def get(self, request):
		...
		if tool.is_default:
			return Response({'status': False, 'message': 'Las herramientas predeterminadas no se pueden desinstalar'})
		...
		if 'go install' in tool.install_command:
			...
			return Response({'status': False, 'message': '¡No se puede desinstalar la herramienta!'})
		...
		return Response({'status': True, 'message': 'Desinstalar el éxito de la herramienta'})
...
class UpdateTool(APIView):
	...
	def get(self, request):
		...
		if not update_command:
			return Response({'status': False, 'message': tool.name + '¡Falta el comando de actualización!No se puede actualizar la herramienta.'})
		...
		try:
			...
			return Response({'status': True, 'message': tool.name + ' actualizado con éxito.'})
		except Exception as e:
			logger.error(str(e))
	...
	def get(self, request):
		...
		if tool_id:
			if not InstalledExternalTool.objects.filter(id=tool_id).exists():
				return Response({'status': False, 'message': 'Herramienta no se encuentra'})
			tool = InstalledExternalTool.objects.get(id=tool_id)
		elif tool_name:
		...
		if not tool.version_lookup_command:
			return Response({'status': False, 'message': 'Comando de búsqueda de versión no proporcionado.'})
		...
		if not version_number:
			return Response({'status': False, 'message': 'Comando de búsqueda de versión no válida.'})
...
class GithubToolCheckGetLatestRelease(APIView):
	...
	def get(self, request):
		...
		if not tool.github_url:
			return Response({'status': False, 'message': 'No se proporciona URL de GitHub, no se puede verificar las actualizaciones'})
