...
function get_interesting_subdomains(project, target_id, scan_history_id) {
	...
	var interesting_subdomain_table = $('#interesting_subdomains').DataTable({
		...
		"oLanguage": {
			...
			"sInfo": "Demostración page _PAGE_ de _PAGES_",
...
function show_subscan_results(subscan_id) {
	...
	fetch(api_url, {
		...
		$('#xl-modal_title').html(`${task_name} Resultados ${response['subscan']['subdomain_name']}`);
		var scan_status = '';
		...
		$('#xl-modal-content').append(`<div>Estado del escaneo: <span class="badge bg-${badge_color}">${scan_status}</span></div>`);
		$('#xl-modal-content').append(`<div class="mt-1">Motor utilizado: <span class="badge bg-primary">${htmlEncode(response['subscan']['engine'])}</span></div>`);
		if (response['result'].length > 0) {
			if (response['subscan']['task'] == 'port_scan') {
				...
				$('#xl-modal-footer').append(`<span class="text-danger">* Puertos poco comunes</span>`);
			} else if (response['subscan']['task'] == 'vulnerability_scan') {
				...
				if (response['result'][0]['directory_files'].length == 0) {
					$('#xl-modal-content').append(`
						...
						<i class="mdi mdi-alert-circle-outline me-2"></i> ${task_name} No pudo obtener ningún resultado.
						</div>
					`);
				} else {
...
function render_vulnerability_in_xl_modal(vuln_count, subdomain_name, result) {
	...
	$('#xl-modal-content').append(`<h5> ${vuln_count} Vulnerabilidades descubiertas en el subdominio ${subdomain_name}</h5>`);
		...
		if (data['status']) {
			...
			for (var result in data['results']) {
				...
				$('#subscan_history_widget').append(`
					...
					${status_badge}
					<span class="">
					Tarea completada ${result_obj.completed_ago} atrás
					</span>
					Tomó ${result_obj.time_taken}
					${error_message}
					</p>
			...
			$('#subscan_history_widget').append(`
					...
					No se han iniciado subscans para ningún subdominio.Puede seleccionar subdominios individuales e iniciar subscans como fuzzing de directorio, escaneo de vulnerabilidad, etc.
					</div>
				`);
		}
	});
...
function get_ip_details(ip_address, scan_id=null, domain_id=null){
	...
	var interesting_badge = `<span class="m-1 badge  badge-soft-danger bs-tooltip" title="Interesting Subdomain">Interesante</span>`;
	...
	// add tab modal title
	$('#modal_title').html('Detalles para la IP: <b>' + ip_address + '</b>');
	...
	$.getJSON(subdomain_url, function(data) {
		...
		$('#modal_content_subdomain').append(`<p>${data['subdomains'].length} Los subdominios están asociados con IP ${ip_address}`);
...
function get_port_details(port, scan_id=null, domain_id=null){
	...
	$.getJSON(ip_url, function(data) {
		$('#modal_ip_ul').empty();
		$('#modal_ip_ul').append(`<p>${data['ips'].length} Las direcciones IP tienen puerto ${port} Abierta`);
		...
		$('#modal_ip_ul').append(`<span class="float-end text-warning">*Dirección IP resaltada son la dirección IP CDN</span>`);
...
function get_tech_details(tech, scan_id=null, domain_id=null){
	...
	var interesting_badge = `<span class="m-1 badge  badge-soft-danger bs-tooltip" title="Interesting Subdomain">Interesante</span>`;
	// render tab modal
	$('.modal-title').html('Detalles para la tecnología: <b>' + tech + '</b>');
	$('#modal_dialog').modal('show');
	...
	$.getJSON(url, function(data) {
		$('#modal-loader').empty();
		$('#modal-content').empty();
		$('#modal-content').append(`${data['subdomains'].length} Los subdominios están usando ${tech}`);
		for (subdomain in data['subdomains']){
			subdomain_obj = data['subdomains'][subdomain];
			badge_color = subdomain_obj['http_status'] >= 400 ? 'danger' : '';
			li_id = get_randid();
		...
		if (response.status) {
			...
			content = `<div class="row mt-3">
				...
				<div class="tab-pane fade active show" id="v-pills-cve-details" role="tabpanel" aria-labelledby="v-pills-cve-details-tab" data-simplebar style="max-height: 600px; min-height: 600px;">
					...
					<span class="badge badge-soft-primary">Asignar:${response.result.assigner}</span>
					<span class="badge badge-outline-primary">Vector CVSS: ${response.result['cvss-vector']}</span>
	...
	fetch('/api/fetch/most_vulnerable/?format=json', {
		...
		if (response.status) {
			...
			for (var res in response.result) {
				...
				$('#most_vulnerable_target_tbody').append(`
					${tr}
						...
						<td>
							<span class="badge badge-outline-danger">${targ_obj.vuln_count} Vulnerabilidades</span>
						</td>
					</tr>
				`);
			}
			...
			$('#most_vulnerable_target_div').append(`
				...
				No pudo encontrar objetivos más vulnerables.
				</br>
				Una vez que se realiza el escaneo de vulnerabilidad, Rengine identificará los objetivos más vulnerables.</div>
			`);
		}
	});
}
...
function get_most_common_vulnerability(slug=null, scan_id=null, target_id=null, ignore_info=false, limit=50){
	...
	fetch('/api/fetch/most_common_vulnerability/?format=json', {
		...
		if (response.status) {
			...
			$('#most_common_vuln_div').append(`
				<div class="mt-4 alert alert-warning">
				No pudo encontrar vulnerabilidades más comunes.
				</br>
				Una vez que se realiza el escaneo de vulnerabilidad, Rengine identificará las vulnerabilidades más comunes.</div>
			`);
		}
	});
}
...
function add_project_modal(){
	Swal.fire({
		...
		preConfirm: (name) => {
			return fetch(`/api/action/create/project?name=${name}`)
				...
				.catch(error => {
					Swal.showValidationMessage(
						`¡Nombre del proyecto duplicado, elija otro nombre del proyecto!`
					)
				})
			},
			...
			if (result.isConfirmed) {
				Swal.fire({
					title: `${result.value.project_name} se crea.`,
					onClose: reloadPage
				})
			}
		});
...
function render_vuln_offcanvas(vuln){
	...
	body += `<p><b>IDENTIFICACIÓN: </b>${vuln.id}</p>`;
	body += `<p><b>Descubierto en: </b>${vuln.discovered_date}</p>`;
	body += `<p><b>URL: </b>
	...
	body += `<p><b>Gravedad: </b>${vuln.severity}<br><b>Type: </b>${vuln.type.toUpperCase()}<br><b>Fuente: </b> ${vuln.source.toUpperCase()}</p>`;
	...
	if (vuln.description) {
		...
		Descripción de la vulnerabilidad <i
	...
	aria-expanded="true" aria-controls="collapseNine">
	Clasificación de vulnerabilidad <i
	class="mdi mdi-chevron-down accordion-arrow"></i>
	</a>
	</h5>
	...
	if (vuln.source == 'nuclei') {
		...
		aria-expanded="true" aria-controls="collapseNine">
		Detalles de la plantilla de núcleos <i
		class="mdi mdi-chevron-down accordion-arrow"></i>
		</a>
		</h5>
	...
	if (vuln.is_gpt_used) {
		body += `<small class="text-muted float-end">(LLM se utilizó para generar detalles de vulnerabilidad).</small>`;
	}
...
async function fetch_gpt_vuln_details(id, title) {
	var loader_title = "Cargando...";
	var text = 'Espere mientras el GPT genere una descripción de vulnerabilidad.'
	try {
...
function render_gpt_vuln_modal(data, title){
	...
	$('#modal-content').empty();
	$('#modal-footer').empty();
	$('#modal_title').html(`Detalle de vulnerabilidad para ${title}`);
...
async function show_attack_surface_modal(id){
	var loader_title = "Cargando...";
	var text = 'Espere mientras el GPT genere la superficie de ataque.'
	try {
		showSwalLoader(loader_title, text);
		const data = await send_gpt__attack_surface_api_request(id);
		Swal.close();
		if (data.status) {
			$('#modal_title').html(`Sugerencia de superficie de ataque para ${data.subdomain_name} (BETA)`);
			$('#modal-content').empty();
...
async function test_hackerone() {
	...
	try {
		const result = await Swal.fire({
			title: 'Configuración de hackerone',
			text: 'Esto probará si sus claves de la API Hackerone están funcionando.',
			icon: 'info',
			showCancelButton: true,
			confirmButtonText: 'Prueba mi tecla API de Hackerone',
			showLoaderOnConfirm: true,
			preConfirm: async () => {
			try {
				...
				if (!response.ok) {
				throw new Error('La respuesta de la red no estaba bien');
				}
		...
		if (result.isConfirmed) {
			...
			text: isWorking
				...
				: 'Tus credenciales de hackerone no funcionan.Consulte su nombre de usuario y/o clave API.',
		...
		await Swal.fire({
			title: 'Error',
			text: 'Se produjo un error inesperado al probar la clave de la API Hackerone.',
			icon: 'error'
		});
	}
}
...
function handleSyncBookmarkedProgramsSwal() {
	Swal.fire({
		...
		icon: 'question',
		showCancelButton: true,
		confirmButtonText: 'Sí, comienza a sincronizar',
		...
		if (result.isConfirmed) {
			...
			.catch((error) => {
				Swal.fire({
					title: 'Falló el inicio del proceso de sincronización',
					text: `Hubo un error al comenzar el proceso de sincronización: ${error.message}`,
					icon: 'error',
