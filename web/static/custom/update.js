...
function check_rengine_update() {
  if (
    ...
    Swal.fire({
      title: "Comprobando la última versión ...",
    ...
    fetch("/api/rengine/update/")
      ...
      .then(function (response) {
        ...
        if (response["description"] == "RateLimited") {
          Swal.fire({
            title: "Oops!",
            text: "Límite de tarifa de GitHub excedido, ¡intente nuevamente en una hora!",
            icon: "error",
          });
          window.localStorage.setItem("update_available", false);
          $(".rengine_update_available").hide();
          ...
          $(".rengine_update_available").hide();
          Swal.fire({
            title: "Actualizar no disponible",
            text: "¡Estás ejecutando la última versión!",
            icon: "info",
          });
        }
      });
...
function update_available(latest_version_number, changelog) {
  ...
  Promise.all([
    ...
    Swal.fire({
      ...
      html: `
          ${changelogStyle}
          <h5>nueva actualización ${latest_version_number} está disponible, siga las instrucciones de actualización.</h5>
...
function showAfterUpdatePopup() {
  ...
  if (lastShownVersion !== currentVersion) {
    ...
    // });
    Swal.fire({
      title: "Gracias por usar",
      text: `¿Le gustaría ver qué hay de nuevo en esta versión??`,
      icon: "info",
      showCancelButton: true,
      confirmButtonText: "Yes, show me",
      cancelButtonText: "No, thanks",
