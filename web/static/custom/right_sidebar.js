function getScanStatusSidebar(project, reload) {
  $.getJSON('/api/scan_status/?project=' + project, function(data) {
    ...
    if (scans['pending'].length > 0){
      ...
      $('#upcoming_scans').html(`<div class="alert alert-info" role="alert">No hay escaneos próximos.</div>`);
    }
    if (scans['scanning'].length > 0){
      // remove display none for current_scan_counter
      $('#current_scan_counter').html(scans['scanning'].length);
      $('#current_scan_count').html(`${scans['scanning'].length} Escaneos actualmente funcionando`)
      for (var scan in scans['scanning']) {
        ...
        $('#currently_scanning').append(`
          ...
          <a href="#" onclick="stop_scan(scan_id=${scan_object.id}, subscan_id=null, reload_scan_bar=true, reload_location=false)" class="btn btn-xs btn-soft-danger waves-effect waves-light mt-1 float-end"><i class="fe-alert-triangle"></i> Detener</a>
          </div>
          </a>
          </div>
          `);
        ...
        $('#currently_scanning').html(`<div class="alert alert-info" role="alert">No se están ejecutando escaneos actualmente.</div>`);
      }
      ...
      if (scans['completed'].length > 0){
        for (var scan in scans['completed']) {
          ...
          if (scan_object.scan_status == 0 ) {
            ...
            color = 'danger';
            status_badge = '<span class="float-end badge bg-danger">Fallida</span>';
          }
          else if (scan_object.scan_status == 3) {
            bg_color = 'bg-soft-danger';
            color = 'danger';
            status_badge = '<span class="float-end badge bg-danger">Abortada</span>';
          }
          else if (scan_object.scan_status == 2){
            bg_color = 'bg-soft-success';
            color = 'success';
            status_badge = '<span class="float-end badge bg-success">Escanear completado</span>';
          }
          ...
          $('#completed').append(`
            ...
            ${status_badge}
            <span class="">
            Escanear completado ${scan_object.completed_ago} atrás
            </span>
            <div>
            <span class="badge-subdomain-count badge badge-soft-info waves-effect waves-light">&nbsp;&nbsp;${scan_object.subdomain_count}&nbsp;&nbsp;</span>
            <span class="badge-endpoint-count badge badge-soft-primary waves-effect waves-light">&nbsp;&nbsp;${scan_object.endpoint_count}&nbsp;&nbsp;</span>
        ...
        $('#completed').html(`<div class="alert alert-info" role="alert">No se han completado recientemente escaneos.</div>`);
      }
      ...
      if (tasks['running'].length > 0){
        $('#current_task_count').html(`${tasks['running'].length} Las tareas se están ejecutando actualmente`)
        for (var task in tasks['running']) {
          ...
          $('#currently_running_tasks').append(`
            ...
            <div class="card-header bg-soft-primary text-primary mini-card-header">
            ${task_name} en <b>${task_object.subdomain_name}</b> Uso del motor <b>${htmlEncode(task_object.engine)}</b>
            </div>
            <div class="card-body mini-card-body">
            <p class="card-text">
            <span class="badge badge-soft-primary float-end scan_status">
            ...
            <span class="">
            Corriendo desde ${task_object.elapsed_time} atrás.
            </span>
            </p>
            <div>
            </div>
            <a href="#" onclick="stop_scan(scan_id=null, subscan_id=${task_object.id}, reload_scan_bar=true, reload_location=false)" class="btn btn-xs btn-soft-danger waves-effect waves-light mt-1 float-end"><i class="fe-alert-triangle"></i> Detener</a>
            </div>
            </a>
            </div>
          `);
        ...
        $('#currently_running_tasks').html(`<div class="alert alert-info" role="alert">No se están ejecutando tareas actualmente.</div>`);
      }
      ...
      if (tasks['completed'].length > 0){
        for (var task in tasks['completed']) {
          ...
          if (task_object.status == 0 ) {
            ...
            color = 'success';
            bg_color = 'bg-soft-success';
            status_badge = '<span class="float-end badge bg-success">Tarea completada</span>';
          }
          ...
          $('#completed_tasks').append(`
            ...
            <div class="card-header ${bg_color} text-${color} mini-card-header">
            ${task_name} on <b>${task_object.subdomain_name}</b> Uso del motor <b>${htmlEncode(task_object.engine)}</b>
            </div>
            <div class="card-body mini-card-body">
            <p class="card-text">
            ${status_badge}
            <span class="">
            Tarea completada ${task_object.completed_ago} atrás
            </span>
            Tomó ${task_object.time_taken}
            ${error_message}
            </p>
            </div>
            </a>
        ...
        $('#completed_tasks').html(`<div class="alert alert-info" role="alert">No se han completado recientemente tareas.</div>`);
      }
      ...
      if (tasks['pending'].length > 0){
        ...
        $('#upcoming_tasks').html(`<div class="alert alert-info" role="alert">No hay tareas futuras.</div>`);
      }

    }).done(function() {
      tippy('.scan_status', {
        content: 'Estado de escaneo',
      });
      ...
      if(reload){
        Snackbar.show({
          text: 'Estado de escaneo recargado.',
          pos: 'top-right',
