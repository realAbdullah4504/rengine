...
def delete_engine(request, slug, id):
    ...
    if request.method == "POST":
        ...
        messages.add_message(
            request,
            messages.INFO,
            'Motor eliminado con éxito!')
    else:
        responseData = {'status': 'false'}
        messages.add_message(
            request,
            messages.ERROR,
            '¡Ups!¡El motor no se pudo eliminar!')
...
def delete_wordlist(request, slug, id):
    ...
    if request.method == "POST":
        ...
        messages.add_message(
            request,
            messages.INFO,
            '¡Lista de palabras eliminada con éxito!')
    else:
        responseData = {'status': 'false'}
        messages.add_message(
            request,
            messages.ERROR,
            '¡Ups!¡La lista de palabras no se pudo eliminar!')
    return http.JsonResponse(responseData)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.add_message(
                request,
                messages.INFO,
                'Palabras clave de búsqueda actualizadas correctamente')
        ...
        if 'gfFileUpload[]' in request.FILES:
            ...
            for gf_file in gf_files:
                ...
                if file_extension == 'json':
                    ...
                    file.close()
                    upload_count += 1
            messages.add_message(request, messages.INFO, f'{upload_count} Archivos GF cargados correctamente')
            ...
            for nuclei_file in nuclei_files:
                ...
                if file_extension in ['yaml', 'yml']:
                    ...
                    upload_count += 1
            if upload_count == 0:
                messages.add_message(request, messages.ERROR, 'Patrón de núcleos no válidos, cargar solo *.yaml extensión')
            messages.add_message(request, messages.INFO, f'{upload_count} Patrones de núcleos cargados con éxito')
            return http.HttpResponseRedirect(reverse('tool_settings', kwargs={'slug': slug}))
            ...
            messages.add_message(request, messages.INFO, '¡Configuración de núcleos actualizada!')
            return http.HttpResponseRedirect(reverse('tool_settings', kwargs={'slug': slug}))
            ...
            messages.add_message(request, messages.INFO, '¡Configuración de subfinder actualizada!')
            return http.HttpResponseRedirect(reverse('tool_settings', kwargs={'slug': slug}))
            ...
            messages.add_message(request, messages.INFO, '¡Configuración de Naabu actualizada!')
            return http.HttpResponseRedirect(reverse('tool_settings', kwargs={'slug': slug}))
            ...
            messages.add_message(request, messages.INFO, '¡Configuración de AMASS actualizada!')
            return http.HttpResponseRedirect(reverse('tool_settings', kwargs={'slug': slug}))
            ...
            messages.add_message(request, messages.INFO, '¡Configuración de TheHarvester actualizada!')
            return http.HttpResponseRedirect(reverse('tool_settings', kwargs={'slug': slug}))
...
def notification_settings(request, slug):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            send_slack_message('Felicidades!Sus servicios de notificación están funcionando.')
            send_lark_message('Felicidades!Sus servicios de notificación están funcionando.')
            send_telegram_message('Felicidades!Sus servicios de notificación están funcionando.')
            send_discord_message('Felicidades!Sus servicios de notificación están funcionando.')
            messages.add_message(
                request,
                messages.INFO,
                'Configuración de notificación actualizada correctamente y se envió un mensaje de prueba.')
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.add_message(
                request,
                messages.INFO,
                '¡Herramienta externa agregada con éxito!')
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.add_message(
                request,
                messages.INFO,
                'Herramienta modificada con éxito')
            return http.HttpResponseRedirect(reverse('tool_arsenal', kwargs={'slug': slug}))
    else:
